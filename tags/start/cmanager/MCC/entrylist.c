/***************************************************************************
    revision             : $Id: entrylist.c,v 1.1.1.1 2003-01-06 15:59:04 tellini Exp $
...
static ULONG EntryList_New( struct IClass *cl, Object *obj, struct opSet *msg )
{
    ...
    if( obj = ( Object * )DoSuperNew( cl, obj,
        ...
        if( tag = FindTagItem( MUIA_EntryList_NoMenu, msg->ops_AttrList ))
            data->NoMenu = tag->ti_Data;
...
static ULONG EntryList_Set( struct IClass *cl, Object *obj, struct opSet *msg )
{
    ...
    if( tag = FindTagItem( MUIA_EntryList_Type, msg->ops_AttrList )) {
        static struct NewMenu UserMenu[] = {
                NM_TITLE, (STRPTR)MSG_USERS, NULL, 0, 0, NULL,
                  ...
                  NM_ITEM, (STRPTR)GAD_TO_PHONE, NULL, 0, 0, (APTR)0,
                    ...
                    NM_SUB, (STRPTR)MSG_BUSINESS_MOBILE, NULL, 0, 0, (APTR)SCP_BUSINESS_MOBILE,
        ...
        static struct NewMenu TelnetMenu[] = {
                NM_TITLE, (STRPTR)MSG_TELNET, NULL, 0, 0, NULL,
                  NM_ITEM, (STRPTR)MSG_TO_CLIP, NULL, 0, 0, (APTR)SCP_TO_CLIP,
                NM_END, NULL, NULL, 0, 0, NULL
        };
    ...
    if( tag = FindTagItem( MUIA_EntryList_Group, msg->ops_AttrList )) {
        ...
        if( tag->ti_Data && ( tag->ti_Data != MUIV_Listtree_Active_Off )) {
            struct CMUser  *user;
    ...
    if( tag = FindTagItem( MUIA_EntryList_CMObj, msg->ops_AttrList ))
    ...
    if( tag = FindTagItem( MUIA_EntryList_NoMenu, msg->ops_AttrList )) {
        ...
        if( !data->NoMenu && data->ContextMenu ) {
...
static ULONG EntryList_Setup( struct IClass *cl, Object *obj, struct opSet *msg )
{
    ...
    if( data->RexxPort ) {
        ...
        data->RexxHandler.ihn_stuff.ihn_sigs = 1L << data->RexxPort->mp_SigBit;
...
static ULONG EntryList_Import( struct IClass *cl, Object *obj, struct MUIP_Import *msg )
{
    ...
    if( id = muiNotifyData( obj )->mnd_ObjectID ) {
        struct OrderPrefs  *prefs;

        if( prefs = (struct OrderPrefs *) DoMethod( msg->dataspace, MUIM_Dataspace_Find, id ))
            data->Order = *prefs;
    }
...
static ULONG EntryList_Export( struct IClass *cl, Object *obj, struct MUIP_Export *msg )
{
    ...
    if( id = muiNotifyData( obj )->mnd_ObjectID )
        DoMethod( msg->dataspace, MUIM_Dataspace_Add, &data->Order, sizeof( data->Order ), id );
            ...
            case ACTION_SCRIPT_ICONIFY: {
                ...
                if( entry ) {
                    ...
                    if(( entry->Type == CME_WWW ) || ( entry->Type == CME_FTP )) {
                        ...
                        DoMethod( _app( obj ), MUIM_Application_PushMethod, obj, 2, MUIM_EntryList_StartScript,
                                  ( entry->Type == CME_WWW ) ? SCP_WWW : ( UseDOpus ? SCP_DOPUSFTP : SCP_FTP ));

                        if( Action != ACTION_SCRIPT ) {
                            STRPTR  base;

                            get( _app( obj ), MUIA_Application_Base, &base );

                            if( strncmp( base, "CONTACTMAN", 10 ) == 0 ) {
...
static ULONG EntryList_StartScript( struct IClass *cl, Object *obj, struct MUIP_EntryList_StartScript *msg )
{
    ...
    if( entry ) {
        ...
        stccpy( Command, (char *)xget( data->CMObj, MUIA_CManager_Path ), sizeof( Command ));
        ...
        if(( script != SCP_PHONE ) && ( script != SCP_MOBILE )) {
            ...
            if( !lock ) {
                ...
                lock = Lock( Scripts[ script ], ACCESS_READ );

                if( lock )
                    strcpy( Command, Scripts[ script ]);
            }
        ...
        switch( entry->Type ) {
                ...
                switch( msg->Script ) {
                    ...
                    case SCP_BUSINESS_PHONE: {
                        ...
                        if( UseDialler ) {
                            ...
                            if( wnd = NewObject( CL_DialWindow->mcc_Class, NULL,
                                                 MUIA_DialWindow_CMObj, data->CMObj,
                                                 TAG_DONE )) {
            ...
            case CME_WWW:
                ...
                DateStamp( &(( struct CMWWW * )entry )->LastVisited );
                break;
            ...
            case CME_FTP:
                ...
                DateStamp( &(( struct CMFTP * )entry )->LastVisited );
                break;
    ...
    while( pos != MUIV_NList_NextSelected_End ) {
        ...
        if(!( ShowEditWindow( _app( obj ), entry ))) {
            ...
            switch( entry->Type ) {
                ...
                case CME_TELNET:
                    Title = WND_EDIT_TELNET;
                    Body  = NewObject( CL_EditTelnet->mcc_Class, NULL, TAG_DONE );
                    ID    = MAKE_ID( 'E', 'D', 'T', 'N' );
                    break;
            }
            ...
            if( Body ) {
                ...
                if( wnd = NewObject( CL_EditWindow->mcc_Class, NULL,
                                     MUIA_EditWindow_CMObj,     data->CMObj,
...
static ULONG EntryList_CopyToClip( struct IClass *cl, Object *obj, Msg msg )
{
    ...
    if( entry ) {
        ...
        switch( entry->Type ) {
            ...
            case CME_TELNET:
                strcpy( string, ((struct CMTelnet *)entry )->Host );
                break;
...
static ULONG EditWindow_New( struct IClass *cl, Object *obj, struct opSet *msg )
{
    ...
    if( obj = ( Object * )DoSuperNew( cl, obj,
        ...
        *data = temp;
        ...
        data->CMObj = (Object *) GetTagData( MUIA_EditWindow_CMObj, NULL, msg->ops_AttrList );
...
static ULONG EditWindow_Get( struct IClass *cl, Object *obj, struct opGet *msg )
{
    ...
    switch ( msg->opg_AttrID ) {
        ...
        case MUIA_EditWindow_IsEditWin:
...
///
/// CloseAllEditWindows
void CloseAllEditWindows( Object *AppObj )
{
    BOOL    loop;
    ...
    do {
        ...
        while( wnd = NextObject( &anchor )) {
            ...
            get( wnd, MUIA_EditWindow_IsEditWin, &ok );

            if( ok ) {

                DoMethod( AppObj, OM_REMMEMBER, wnd );
