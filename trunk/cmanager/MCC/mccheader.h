...
const char UserLibName[] = CLASS;
const char UserLibId[]   = UserLibID;
...
#define MCCD(flags,x) { x; }
#define mccbug kprintf("%-12.12s %-17.17s: ",__FILE__,__FUNC__); kprintf
#define mccbugg kprintf
...
struct LibraryHeader * ASM SAVEDS LibInit(REG(a0) BPTR Segment)
{
        ...
        {
                MCCD(DBF_LIB,mccbug("\7MakeLibrary() failed\n"));
        }
...
struct LibraryHeader * ASM SAVEDS LibOpen(REG(a6) struct LibraryHeader *base)
{
        ...
        else
        {
                rc = NULL;
                base->lh_Library.lib_OpenCnt--;
                MCCD(DBF_LIB,mccbug("\7UserLibOpen() failed\n"));
        }

        ReleaseSemaphore(&base->lh_Semaphore);

        MCCD(DBF_LIB,mccbug("exiting\n"));
...
BOOL ASM SAVEDS UserLibOpen(REG(a6) struct Library *base)
{
        ...
        MCCD(DBF_CLASS,mccbug("fail.: %08lx %s\n",base,base->lib_Node.ln_Name));
